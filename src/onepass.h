...
{   
	...
	//tt.push_back( std::chrono::steady_clock::now() );
        ...
        for(int j=bsize-1; j>=0; j--){
            ...
            for (int i1 = 0; i1 < dim; i1++) {
                for (int i2 = 0; i2 < dim; i2++) {
                    // localsub(i1, i2, bsize-1-j) = aniso(i1, i2, NNarray(i,j)-1);
                    localsub[i1 + dim * (i2 + dim * (bsize-1-j))] = aniso[i1 + dim * (i2 + dim * (NNarray(i,j)-1))];
                }
            }
            // localsub( _, _, bsize-1-j ) = aniso( _, _, NNarray(i,j)-1 );
            for(int k=0;k<dim;k++){ locsub(bsize-1-j,k) = locs( NNarray(i,j)-1, k ); }
            if(profbeta){ 
                for(int k=0;k<p;k++){ X0(bsize-1-j,k) = X( NNarray(i,j)-1, k ); } 
            }
        ...
        arma::cube dcovmat;
        if(grad_info){ 
            dcovmat = p_d_covfun[0]( covparms, locsub, localsub ); 
        }

        // covmat.print();
        // printf("\n");
        // dcovmat.print();
        covmat += 1e-6 * eye(bsize, bsize);
...
void synthesize_local(
    ...
    compute_pieces_local(
        covparms_c, covfun_name, locs_c, NNarray_c, y_c, X_c,
        &XSX, &ySX, &ySy, &logdet, &dXSX, &dySX, &dySy, &dlogdet, &ainfo,
        profbeta, grad_info, aniso
    );
